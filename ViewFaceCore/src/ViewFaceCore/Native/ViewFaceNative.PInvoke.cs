...
internal static partial class ViewFaceNative
{
    ...
    [DllImport(BRIDGE_LIBRARY_NAME, EntryPoint = "SetModelPath", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
    ...
    public static void SetModelPath(string path)
    {
        ...
        // 注意：GetModelPath() 的 P/Invoke 在 .NET 8 + Windows (CharSet.Ansi) 下可能 AccessViolation
        // 由于 SetModelPathWindows 已成功调用，直接设置 _modelPath 并跳过验证
        _modelPath = path;
    }
    ...
    [DllImport(BRIDGE_LIBRARY_NAME, EntryPoint = "GetModelPath", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
    ...
    /// <returns></returns>
    [DllImport(BRIDGE_LIBRARY_NAME, EntryPoint = "MaskDetector", CallingConvention = CallingConvention.Cdecl)]
    ...
    [DllImport(BRIDGE_LIBRARY_NAME, EntryPoint = "FaceAntiSpoofing", CallingConvention = CallingConvention.Cdecl)]
